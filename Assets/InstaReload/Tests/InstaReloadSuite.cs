...
namespace Nimrita.InstaReload.Tests
{
    ...
    public sealed class InstaReloadSuite : MonoBehaviour
    {
        ...
        private void Evaluate()
        {
            ...
            Check(t, "generic method <string>", GenericMethod("x"), Expect.Stale);
            Check(t, "generic method <int>", GenericMethod(1), Expect.Stale);
            Check(t, "generic class <string>", new Boxed<string>().Read(), Expect.Stale);
            Check(t, "generic class <int>", new Boxed<int>().Read(), Expect.Stale);
            Check(t, "generic class new List<T>", new Boxed<int>().WithOpenList(), Expect.Stale);

            var combos = new Combos();
            Check(t, "combo two type params", combos.TwoParams(1, "x"), Expect.Stale);
            Check(t, "combo where T : struct", combos.StructOnly(7), Expect.Stale);
            Check(t, "combo nested generic arg", combos.Nested(new List<int>()), Expect.Stale);
    ...
    public sealed class Combos
    {
        ...
        public string ConstructsOwnGeneric()
        {
            ...
            return boxed != null ? Marker : "CONSTRUCT-NULL";
        }
